...
import { useGetQuestionnaireQuery, useUpdateQuestionnaireMutation } from '../../store/services/questionnaire/questionnaireApi';
...
const UpdateQuestionnaire = ({ navigation }: { navigation: any }) => {
    ...
    const { user } = useAuth();
    const { data: questionnaireData, isLoading: isFetching } = useGetQuestionnaireQuery(id);
    const [updateQuestionnaire, { isLoading }] = useUpdateQuestionnaireMutation();
    ...
    useEffect(() => {
        if (questionnaireData) {
            ...
            setFormData({
                ...
                idCardAvailable: data.idCardAvailable || '',
                cardNumber: data.cardNumber || '',
                religion: data.religion || '',
                religion_input: data.religion_input || '',
                ...
                firstDegreeRelativeOralCancer: data.firstDegreeRelativeOralCancer || '',
                height: data.height || 0,
                diabetes: data.diabetes || '',
                hypertension: data.hypertension || '',
                dietHistory: data.dietHistory || '',
                fruitsConsumption: data.fruitsConsumption || '',
                vegetableConsumption: data.vegetableConsumption || '',
                habitHistory: data.habitHistory || '',
                tobaccoChewer: data.tobaccoChewer || '',
                tobaccoType: data.tobaccoType || '',
                discontinuedHabit: data.discontinuedHabit || '',
                durationOfDiscontinuingHabit: data.durationOfDiscontinuingHabit || '',
                otherConsumptionHistory: data.otherConsumptionHistory || '',
                alcoholConsumption: data.alcoholConsumption || '',
                smoking: data.smoking || '',
                oralCavityExamination: data.oralCavityExamination || '',
                presenceOfLesion: data.presenceOfLesion || '',
                reductionInMouthOpening: data.reductionInMouthOpening || '',
                suddenWeightLoss: data.suddenWeightLoss || '',
                presenceOfSharpTeeth: data.presenceOfSharpTeeth || '',
